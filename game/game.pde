...
static int TYPE_HALF21 = 6;
static int TYPE_HALF22 = 7;
...
class Entity {
  ...
  Entity(float _x, float _y, float _xs, float _ys, int _type, int _contains) {
    ...
    if (_contains == TYPE_HALF21 && x > width/2) _contains = TYPE_HALF22;
    if (_contains == TYPE_HALF22 && x < width/2) _contains = TYPE_HALF21;
  ...
  boolean insidescreen() {
    if ((x-(height/6) < 0-height/3) || (y-(height/6) < 0-height/3) || (x+(height/6) > width+height/3) || (y+(height/6) > height+height/3)) return false;
    return true;
  }
  ...
  boolean hit(float tx, float ty) {
    ...
    if (type == TYPE_BUBBLE && dist(x, y, tx, ty)<height/6) return true;
    if (type != TYPE_BUBBLE && type < TYPE_HALF11 && dist(x, y, tx, ty)<height/8) { 
      return true;
    }
    ...
    if (type >= TYPE_HALF11 && dist(x, y, tx, ty)<height/6) { 
      last_tx = tx; 
      last_ty = ty; 
      drag = true; 
      if (type == TYPE_HALF11 || type == TYPE_HALF21) isdragging1 = true; 
      if (type == TYPE_HALF12 || type == TYPE_HALF22) isdragging2 = true; 
      if (x < width/2 && dragside == -1) dragside = 0; 
      if (x > width/2 && dragside == -1) dragside = 1; 
      return true;
    }
  ...
  void update() {
    ...
    if (exptimer > 0. && type != TYPE_BUBBLE) exptimer -= dt*0.0075;
    if (exptimer <= 0. && type >= TYPE_HALF11) { 
      alive = false; 
      life--; 
      bg_flash = 2.0; 
      dmg_flash = 1; 
    ...
    if (drag && dragside == 0 && x >= width/2-16) {
      drag = false; 
      locked = true; 
      isdragging1 = false;
    }
    if (drag && dragside == 1 && x <= width/2+16) {
      drag = false; 
      locked = true; 
      isdragging2 = false;
    }
  ...
  void render() {
    ...
    if (type == TYPE_BUBBLE) {
      ...
      ellipse(x, y, height/3+sin(millis()*0.004+x)*16, height/3+cos(millis()*0.0052+x)*16);
    }
    ...
    else if (contains == TYPE_HALF21) image(image_kisu21, x+xo, y+yo);
    else if (contains == TYPE_HALF22) image(image_kisu22, x+xo, y+yo);
    popMatrix();
...
void setup() {
  ...
  image_kisu21 = loadImage("kisu2_1.png");
  image_kisu22 = loadImage("kisu2_2.png");
...
void resetLevel() {
  ...
  int lvl = level % 2;
  if (lvl == 0) music = new SoundFile(this, "sport.mp3");
  if (lvl == 1) music = new SoundFile(this, "hydro.mp3");
  if (lvl == 2) music = new SoundFile(this, "nonsense.mp3");
      ...
      for (int j=0; j<ents.size(); j++) {
        ...
        if (ent.drag) {
          if ((ent.dragside == 0 && ent.x < width/2) || (ent.dragside == 1 && ent.x > width/2)) { 
            if (dist(ent.x, ent.y, tx, ty) > height/3) { 
              ent.drag = false;
...
void drawShapes() {
  for (int i=0; i<ents.size(); i++) {
    ...
    if (ent.insidescreen()) ent.render();
  }
}
...
void drawText() {
  ...
  if (gamestarttimer >= 0.0 && gamestarttimer < 2.5) {
    ...
    text("code by visy", width/2, height-150);
    text("graphics by Heini", width/2, height-100);
    text("music by mathgrant (CC)", width/2, height-50);
  }

  if (gamestarttimer >= 2.5 && gamestarttimer < 6.0) {
    background(255);
...
void levelLogic() {
  ...
  for (int j=0; j<ents.size(); j++) {
    ...
    for (int k=0; k<ents.size(); k++) {
      ...
      if (!ent.foundpair && !ent2.foundpair && ent.locked && ent2.locked && dist(ent.x, ent.y, ent2.x, ent2.y) < 300 && ent.alive && ent2.alive && ent.type != ent2.type) {

        if ((ent.type == TYPE_HALF11 && ent2.type == TYPE_HALF12) || 
          (ent.type == TYPE_HALF12 && ent2.type == TYPE_HALF11) ||
          (ent.type == TYPE_HALF21 && ent2.type == TYPE_HALF22) ||
          (ent.type == TYPE_HALF22 && ent2.type == TYPE_HALF21) ) {
          ent.foundpair = true;
          ent2.foundpair = true;
          ent.drag = false;
          ent2.drag = false;
  ...
  spawntime += dt*(0.005*bubblespeed);
  ...
  if (spawntime > abs((5.0-(level*0.5)))) {
    ...
    if (spawnside == 1) {
      if (toporbottom == 0) ents.add(new Entity(width/8+random(0, width/4), -160+random(0, 10), 0, 4+random(0, 2+level), TYPE_BUBBLE, TYPE_BOMB+int(random(0, 7))));
      else ents.add(new Entity(width/8+random(0, width/4), height+120+random(0, 10), 0, -4-random(0, 2+level), TYPE_BUBBLE, TYPE_BOMB+int(random(0, 7))));
    } else {
      if (toporbottom == 0) ents.add(new Entity(width/2+width/4+random(0, width/8), -160+random(0, 10), 0, 2+random(0, 2+level), TYPE_BUBBLE, TYPE_BOMB+int(random(0, 7))));
      else ents.add(new Entity(width/2+width/8+random(0, width/4), height+120+random(0, 10), 0, -2-random(0, 2+level), TYPE_BUBBLE, TYPE_BOMB+int(random(0, 7))));
    }
  }
}
